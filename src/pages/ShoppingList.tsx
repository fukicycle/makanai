...
export const ShoppingList: React.FC = () => {
  ...
  if (!menuData || !menuData.shoppingList || menuData.shoppingList.length === 0) {
    return (
      <div className="flex flex-col h-full overflow-hidden p-4 space-y-6">
        {/* ヘッダー */}
        <div className="border-b border-orange-200/50 pb-4 text-left shrink-0">
          <h2 className="text-xl font-bold text-orange-900 font-serif flex items-center gap-2">
            <ShoppingBag className="w-5 h-5 text-orange-600" />
            買い物リスト
          </h2>
          <p className="text-xs text-stone-500 mt-1 font-serif">一週間分の必要な買い物をまとめて確認できます。</p>
        </div>

        {/* 未生成時の画面 */}
        <div className="flex-grow flex flex-col justify-center items-center">
          <div className="glass-card rounded-3xl p-8 text-center max-w-md mx-auto space-y-4 border border-white/80 animate-soft-fade">
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden animate-soft-fade">
      {/* 固定ヘッダー */}
      <div className="p-4 border-b border-orange-200/50 text-left bg-gradient-to-b from-amber-50/10 to-transparent shrink-0 flex flex-col gap-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          {/* 進捗バー */}
          <div className="w-full sm:w-48 bg-stone-100 p-2.5 rounded-xl border border-stone-200/40">
        ...
        {/* 注意書き */}
        <div className="p-2.5 bg-orange-50/50 rounded-xl border border-orange-200/20 flex items-start gap-2">
          <Info className="w-3.5 h-3.5 text-orange-600 shrink-0 mt-0.5" />
          <p className="text-[10px] text-stone-500 leading-normal font-serif">
                ...
                {items.map((item) => (
                  ...
                  >
                    <div className="flex items-center gap-2 text-left">
                      ...
                      <span 
                        className={`text-custom-readable font-serif transition-all ${
                          item.checked 
                    ...
                    <span 
                      className={`text-custom-readable font-sans font-bold transition-all shrink-0 ${
