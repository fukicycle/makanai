...
export const Home: React.FC = () => {
  ...
  const handleGenerate = async (overrideFridgeContents?: string) => {
    setGenerating(true);
    setErrorMsg(null);
    const contentsToUse = typeof overrideFridgeContents === 'string' ? overrideFridgeContents : fridgeContents;
    try {
      ...
      if (err instanceof Error) {
        ...
        setErrorMsg('献立の生成中に予期せぬ不具合が発生いたしました。');
      }
    } finally {
      setGenerating(false);
    }
  ...
  const dayOrderColors = [
    ...
    'border-l-yellow-600 bg-yellow-50/20', // 木
    'border-l-red-500 bg-red-50/20', // 金
    'border-l-sky-500 bg-blue-50/20', // 土
    'border-l-rose-500 bg-rose-50/20', // 日
  ];

  return (
    <div className="space-y-6 pb-24">
      {/* 上部ヘッダー */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-orange-200/50 pb-4">
        <div>
          <h2 className="text-xl font-bold text-orange-900 font-serif flex items-center gap-2">
            <Utensils className="w-5 h-5 text-orange-600" />
            一週間の献立
          </h2>
          <p className="text-xs text-stone-500 mt-1">ご家族の健やかな毎日のために、まかなひが思案いたします。</p>
        </div>

        {/* 家族カロリー概要 */}
        <div className="flex items-center gap-3 px-4 py-2 bg-orange-50/70 border border-orange-200/40 rounded-xl">
          <Users className="w-4 h-4 text-orange-600" />
          <div className="text-left">
            <p className="text-[10px] text-stone-500 font-serif">推定必要夕食カロリー (合計)</p>
            <p className="text-sm font-bold text-orange-800 font-sans">{targetCal} <span className="text-xs font-serif font-normal">kcal / 日</span></p>
          </div>
        </div>
      </div>
      ...
      {!menuData && !generating && (
        <motion.div 
          ...
          className="glass-card rounded-3xl p-6 md:p-8 text-center max-w-md mx-auto space-y-6 border border-white/80 shadow-xl"
        >
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-stone-500 leading-relaxed max-w-xs mx-auto">
              ご入力いただいた家族構成に基づき、栄養バランスに長けた1週間分の和風夕食をGeminiが提案いたします。
            </p>
          </div>

          {/* 現在のこしらえ加減の簡易インジケータ */}
          <div className="p-3 bg-stone-100/60 rounded-xl border border-stone-200/30 flex items-center justify-between text-left">
            <div className="flex items-center gap-2">
              <ChefHat className="w-4 h-4 text-orange-600 shrink-0" />
              <div className="leading-tight">
                <span className="text-[9px] text-stone-400 block font-serif">現在のこしらえ加減</span>
          ...
          {/* 冷蔵庫ののぞき（余り物）入力欄 */}
          <div className="space-y-3 text-left border-t border-stone-200/40 pt-4">
            ...
            {/* 食材のクイック選択 */}
            <div className="space-y-2 bg-stone-50/50 p-2.5 rounded-xl border border-stone-200/30">
              <p className="text-[10px] text-stone-500 font-serif font-semibold">食材をぽちっと選べます：</p>
              <div className="space-y-1.5">
                {Object.entries(commonIngredients).map(([category, items]) => (
                  <div key={category} className="flex flex-wrap items-center gap-1">
                    <span className="text-[8px] font-bold text-stone-400 font-serif w-12 shrink-0">{category}</span>
                      ...
                      {items.map(item => {
                        ...
                        return (
                          <button
                            ...
                            className={`px-2 py-0.5 rounded-full text-[10px] font-serif border transition-all cursor-pointer ${
                              isSelected
          ...
          {errorMsg && (
            <div className="p-3 bg-red-50 text-red-600 border border-red-200 rounded-xl text-xs text-left">
              {errorMsg}
            </div>
          )}
          ...
          <button
            ...
            className="w-full py-3.5 bg-orange-600 hover:bg-orange-700 text-sm font-bold text-white rounded-2xl shadow-lg shadow-orange-500/20 hover:shadow-orange-500/30 transition-all flex items-center justify-center gap-2"
          >
      ...
      {/* 生成中ローディング */}
      {generating && (
        <div className="glass-card rounded-3xl p-10 text-center max-w-md mx-auto space-y-6 border border-white/80 min-h-[300px] flex flex-col justify-center items-center">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500 leading-relaxed">
              ご家族に必要な栄養とカロリーを計算し、<br />温かみのある一週間分のレシピを整えております。
            </p>
          </div>
          <p className="text-[10px] text-stone-400">※これには10秒〜20秒ほどお時間を要する場合がございます。</p>
        </div>
      )}
        ...
        <div className="space-y-6 animate-soft-fade">
          ...
          <div className="flex justify-between items-center text-xs text-stone-400 font-sans px-1">
            ...
            <button
              ...
              className="flex items-center gap-1 text-orange-600 hover:text-orange-700 transition-colors font-bold font-serif"
            >
          ...
          {menuData.makeAheadSideDish && (
            <motion.div
              ...
              className="glass-card p-5 rounded-3xl border border-emerald-200/50 bg-emerald-50/10 text-left flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 cursor-pointer hover:shadow-lg transition-all"
            >
              ...
              <button
                ...
                className="w-full sm:w-auto py-2 px-5 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-emerald-700/10 shrink-0 font-serif"
              >
                こしらえ方を見る
              </button>
            </motion.div>
          ...
          {/* 常備菜詳細モーダル */}
          <AnimatePresence>
            {showSideDishModal && menuData.makeAheadSideDish && createPortal(
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              ...
              >
                <motion.div
                  ...
                  className="w-full max-w-lg bg-white border border-stone-200/50 shadow-2xl rounded-3xl p-6 text-left space-y-4 max-h-[85vh] overflow-y-auto"
                ...
                >
                  ...
                  <div className="space-y-4 text-xs text-stone-700 leading-relaxed">
                    ...
                    <div className="space-y-3 bg-stone-50/50 p-3 rounded-xl border border-stone-200/30">
                      <p className="text-[10px] text-stone-500 font-serif font-semibold">食材を選べます（ぽちっと押すと追加・削除されます）：</p>
                      <div className="space-y-2">
                        {Object.entries(commonIngredients).map(([category, items]) => (
                          <div key={category} className="space-y-1">
                            <span className="text-[9px] font-bold text-stone-400 font-serif block">{category}</span>
                  ...
                  <div className="flex justify-end gap-2 border-t border-stone-200/40 pt-3">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        handleGenerate(tempFridgeContents);
                      }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {menuData.days.map((day, index) => (
              <motion.div
                ...
                className={`glass-card p-5 rounded-2xl border-l-4 ${dayOrderColors[index % 7]} text-left cursor-pointer flex flex-col justify-between hover:shadow-lg transition-all border border-stone-200/40`}
              >
                <div>
